...
# - tsv_file: TSV file with positions of sequences to be evaluated.
#
# Parameters:
...
# python multiGPU-genomic_shifted_permutation.py params.json model_file.h5 positions.tsv
...
def main():
    ...
    for pi in range(options.processes):
        if not options.restart or not job_started(options, pi):
            ...
            cmd += (
                "python ${SLURM_SUBMIT_DIR}/genomic_shifted_permutation.py %s %s %d;"
                % (
                    options_pkl_file,
                    " ".join(args),
                    pi,
