...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/genomic_disruption_profile/input_data/mm10_chr1.0_start_windows.tsv"
out_dir="/scratch1/smaruj/sliding_disruption/human_model" 
...
do 
    ...
    python multiGPU-genomic_disruption_sliding_profile.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
