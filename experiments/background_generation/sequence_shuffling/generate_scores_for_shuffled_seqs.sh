...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/background_generation/sequence_shuffling/shuffled_600seqs.tsv"
chrom_sizes="/project/fudenber_735/genomes/mm10/mm10.fa.sizes"
out_dir="/scratch2/smaruj/background_directory/shuffling"
...
do 
    ...
    python multiGPU-generate_scores_for_shuffled_seqs.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
