...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/background_generation/sequence_shuffling/shuffled_600seqs_k8.tsv"
chrom_sizes="/project/fudenber_735/genomes/mm10/mm10.fa.sizes"
out_dir="/scratch1/smaruj/shuffled_unshuffled/unshuffled"
models="1 2 3" # this is a string with space seperated model numbers examples "0 1 2" or "4 5 6" 
batch_size=8 
max_proc=1
processes=1
stats="SCD"
...
do 
    ...
    python multiGPU-generate_scores_for_unshuffled_insertions.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
