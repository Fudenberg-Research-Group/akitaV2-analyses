...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/virtual_insertion_single_mutagenesis/input_data/CTCFs_jaspar_filtered_mm10_top100_single_mutagenesis.tsv"
out_dir="/scratch2/smaruj/single_mutagenesis/single_mutagenesis" 
...
do 
    ...
    python multiGPU-virtual_insertion_single_mutagenesis.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
