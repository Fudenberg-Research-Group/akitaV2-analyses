...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/virtual_insertion_spacing/input_data/divergent_orientation/CTCFs_jaspar_filtered_mm10_top20_logspacing_bg7_D.tsv"
out_dir="/scratch1/smaruj/spacing_divergent/spacing_D_bg7" 
...
do 
    ...
    python multiGPU-virtual_insertion_logspacing.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
