...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/virtual_insertion_number/input_data/CTCFs_jaspar_filtered_mm10_number_R40_bg8.tsv"
out_dir="/scratch1/smaruj/vary_number_more/right40_bg8" 
...
do 
    ...
    python multiGPU-virtual_number_insertion.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
