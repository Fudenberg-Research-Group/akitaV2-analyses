# Description:
# This script performs predictions using a trained deep learning model on DNA sequences and writes the results to HDF5 files.
# It supports single and multi-GPU execution and handles various options for plotting and saving predictions.
#
# Inputs:
# - <params_file>: JSON file containing parameters for the analysis.
# - <model_file>: Path to the trained model file.
# - <motifs_file>: File with motif positions in TSV format.
# - Optionally, <options_pkl_file> for multi-GPU execution.
#
# Optional Parameters:
...
#
# Outputs:
# - HDF5 file containing the prediction statistics and possibly contact maps.
# - Separate directories and files for multi-GPU execution if specified.
#
# Example command-line usage:
# python virtual_insertion_pairwise_mutagenesis.py params.json model.h5 motifs.tsv -f genome.fa --batch-size 8 --save-maps --out-dir ./results
...
from akita_utils.seq_gens import pairwise_mutagenesis_seqs_gen
...
def main():
    ...
    preds_stream = stream.PredStreamGen(
        seqnn_model,
        pairwise_mutagenesis_seqs_gen(seq_coords_df, background_seqs, genome_open),
        batch_size,
    )
