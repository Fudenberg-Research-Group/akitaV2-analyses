...
tsv_file="/home1/smaruj/akitaX1-analyses/experiments/virtual_insertion_flank_core_compatibility/input_data/CTCFs_jaspar_filtered_mm10_sampled_high_medium_lowbg_2.tsv"
out_dir="/scratch2/smaruj/compatibility/same_sample_bg2" 
...
do 
    ...
    python multiGPU-virtual_insertion_flank_core_compatibility.py "${params_file}" "${model_file}" "${tsv_file}" -f "${genome_fasta}" -o "${this_out_dir}" --stats "${stats}" --batch-size "${batch_size}" -p "${processes}" --max_proc "${max_proc}" --name "${name}" --time "${time}"
    sleep 15
done
